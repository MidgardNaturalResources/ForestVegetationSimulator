...
C----------
   15 CONTINUE
      ...
      IF(MCFT(I).GT.0.0 .AND. LCVOLS) THEN
C----------
...
C----------
        ...
        IF(CFLA0(ISPC).EQ.0.0 .AND. CFLA1(ISPC).EQ.1.0) THEN
          VOLCOR=MCFT(I)
        ELSE
          VOLCOR=EXP(CFLA0(ISPC)+CFLA1(ISPC)*ALOG(MCFT(I)))
        ENDIF
        DLLMOD=NINT(((MCFT(I)-VOLCOR)/MCFT(I)) * 100.)
...
C----------
      IF(ICDF.LT.99) THEN
        MCFT(I)=MCFT(I)*(1.-FLOAT(ICDF)/100.)
      ELSE
        MCFT(I)=0.
      ENDIF
      IF(IPASS .EQ. 1) SPCMC(ISPC,IM)=SPCMC(ISPC,IM)+MCFT(I)*P
C----------
C  CALL ECON EXTENSION TO PROCESS VOLUME INFORMATION
C----------
      IF (MCFT(I) .GT. 0.0) CALL ECVOL(IT,LOGDIA,LOGVOL,.TRUE.)
...
C----------
      ...
      WRITE(JOSTND,9000)I,VN,MCFT(I),WK5(I),CFV(I),
     &     SPCAC(ISPC,1),SPCAC(ISPC,2),SPCAC(ISPC,3),SPCCC(ISPC,1),
     &     SPCCC(ISPC,2),SPCCC(ISPC,3),ISPC,D,H
 9000 FORMAT(' IN VOLS, I=',I4,' VN=',E15.6,' MCFT=',E15.6,/,
     &  '  WK5=',E15.6,' CFV=',E15.6,
